package mayachain

import (
	"context"
	"fmt"
	"strconv"

	"github.com/btcsuite/btcd/btcutil"
	"github.com/btcsuite/btcd/chaincfg"
	"github.com/btcsuite/btcd/txscript"
	"github.com/btcsuite/btcd/wire"
	btc_swap "github.com/vultisig/app-recurring/internal/btc"
	"github.com/vultisig/app-recurring/internal/utxo/address"
	"github.com/vultisig/vultisig-go/common"
)

// ProviderBtc implements the Bitcoin swap provider for MayaChain
type ProviderBtc struct {
	client *Client
}

// NewProviderBtc creates a new Bitcoin provider for MayaChain swaps
func NewProviderBtc(client *Client) *ProviderBtc {
	return &ProviderBtc{
		client: client,
	}
}

func (p *ProviderBtc) validateBtc(from btc_swap.From, to btc_swap.To) error {
	if to.Chain == common.Bitcoin {
		return fmt.Errorf("btc: can't swap BTC to BTC")
	}

	_, err := parseMayaNetwork(to.Chain)
	if err != nil {
		return fmt.Errorf("btc: unsupported 'to' chain for MayaChain: %w", err)
	}

	// Validate from address is a BTCAddress with a supported native type
	btcAddr, ok := from.Address.(*address.BTCAddress)
	if !ok {
		return fmt.Errorf("btc: unsupported 'from' address type: expected BTCAddress")
	}
	switch btcAddr.Native().(type) {
	case *btcutil.AddressWitnessScriptHash,
		*btcutil.AddressWitnessPubKeyHash,
		*btcutil.AddressPubKeyHash,
		*btcutil.AddressScriptHash,
		*btcutil.AddressTaproot:
	default:
		return fmt.Errorf("btc: unsupported 'from' address type")
	}

	return nil
}

// SatsPerByte returns the current fee rate for Bitcoin transactions from MayaChain
func (p *ProviderBtc) SatsPerByte(ctx context.Context) (uint64, error) {
	info, err := p.client.getInboundAddresses(ctx)
	if err != nil {
		return 0, fmt.Errorf("btc: failed to get inbound addresses: %w", err)
	}

	for _, addr := range info {
		if addr.Chain == btc {
			satsPerByte, er := strconv.ParseUint(addr.GasRate, 10, 64)
			if er != nil {
				return 0, fmt.Errorf("btc: failed to parse gas rate: %w", er)
			}
			return satsPerByte, nil
		}
	}
	return 0, fmt.Errorf("btc: no gas info found for BTC")
}

// ChangeOutputIndex returns the index of the change output in swap transactions
func (p *ProviderBtc) ChangeOutputIndex() int {
	return 1
}

// MakeOutputs creates transaction outputs for a MayaChain swap
func (p *ProviderBtc) MakeOutputs(
	ctx context.Context,
	from btc_swap.From,
	to btc_swap.To,
) (uint64, []*wire.TxOut, error) {
	if err := p.validateBtc(from, to); err != nil {
		return 0, nil, fmt.Errorf("btc: invalid swap: %w", err)
	}

	toAsset, err := MakeMayaAsset(ctx, p.client, to.Chain, to.AssetID)
	if err != nil {
		return 0, nil, fmt.Errorf("btc: failed to convert maya asset: %w", err)
	}

	quote, err := p.client.GetQuote(ctx, QuoteSwapRequest{
		FromAsset:         string(btc) + "." + string(btc),
		ToAsset:           toAsset,
		Amount:            fmt.Sprintf("%d", from.Amount),
		Destination:       to.Address,
		StreamingInterval: DefaultStreamingInterval,
		StreamingQuantity: DefaultStreamingQuantity,
	})
	if err != nil {
		return 0, nil, fmt.Errorf("btc: failed to get quote: %w", err)
	}

	inboundAddr, err := btcutil.DecodeAddress(quote.InboundAddress, &chaincfg.MainNetParams)
	if err != nil {
		return 0, nil, fmt.Errorf("btc: failed to decode inbound address: %w", err)
	}

	inboundScript, err := txscript.PayToAddrScript(inboundAddr)
	if err != nil {
		return 0, nil, fmt.Errorf("btc: failed to create inbound script: %w", err)
	}

	changeScript, err := from.Address.PayToAddrScript()
	if err != nil {
		return 0, nil, fmt.Errorf("btc: failed to create change script: %w", err)
	}

	memoScript, err := txscript.NullDataScript([]byte(quote.Memo))
	if err != nil {
		return 0, nil, fmt.Errorf("btc: failed to create memo script: %w", err)
	}

	expectedOut, err := strconv.ParseUint(quote.ExpectedAmountOut, 10, 64)
	if err != nil {
		return 0, nil, fmt.Errorf("btc: failed to parse expected amount out: %w", err)
	}

	dustThreshold, err := strconv.ParseUint(quote.DustThreshold, 10, 64)
	if err != nil {
		return 0, nil, fmt.Errorf("btc: failed to parse dust threshold: %w", err)
	}

	if from.Amount < dustThreshold {
		return 0, nil, fmt.Errorf("btc: amount %d below dust threshold %d", from.Amount, dustThreshold)
	}

	outputs := []*wire.TxOut{
		{
			Value:    int64(from.Amount),
			PkScript: inboundScript,
		},
		{
			Value:    0, // Change amount will be calculated later
			PkScript: changeScript,
		},
		{
			Value:    0,
			PkScript: memoScript,
		},
	}

	return expectedOut, outputs, nil
}
