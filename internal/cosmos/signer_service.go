...
// SignerService handles Cosmos transaction signing and broadcasting
...
// SignAndBroadcast signs a Cosmos transaction and broadcasts it
func (s *SignerService) SignAndBroadcast(
	...
	if err != nil {
		return "", fmt.Errorf("cosmos: failed to build keysign request: %w", err)
	}

	signatures, err := s.signer.Sign(ctx, keysignRequest)
	if err != nil {
		return "", fmt.Errorf("cosmos: failed to get signature: %w", err)
	}

	signedTxBytes, err := s.sdk.Sign(txData, signatures, pubKey)
	if err != nil {
		return "", fmt.Errorf("cosmos: failed to sign transaction: %w", err)
	}

	_, err = s.sdk.Broadcast(ctx, signedTxBytes)
	if err != nil {
		return "", fmt.Errorf("cosmos: failed to broadcast transaction: %w", err)
	}

	// Cosmos txHash is SHA256 of signed tx bytes (uppercase hex)
...
func (s *SignerService) buildKeysignRequest(
	...
	// For Cosmos, the hash to sign is SHA256 of the sign bytes
	...
	if err != nil {
		return types.PluginKeysignRequest{}, fmt.Errorf("cosmos: failed to create tx: %w", err)
	}
