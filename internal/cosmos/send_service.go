...
// SendService handles building Cosmos send transactions
...
// BuildTransfer builds an unsigned Cosmos bank send transaction
...
func (s *SendService) BuildTransfer(
	...
	if err != nil {
		return nil, nil, fmt.Errorf("cosmos: failed to get account info: %w", err)
	}
...
// BuildTransferWithAccountInfo builds an unsigned Cosmos bank send transaction with explicit account info
func (s *SendService) BuildTransferWithAccountInfo(
	...
	if err != nil {
		return nil, nil, fmt.Errorf("cosmos: invalid from address: %w", err)
	}

	toAddr, err := cosmostypes.AccAddressFromBech32(to)
	if err != nil {
		return nil, nil, fmt.Errorf("cosmos: invalid to address: %w", err)
	}

	// Create send message
	amount := cosmostypes.NewCoins(cosmostypes.NewCoin(AtomDenom, math.NewIntFromUint64(amountUatom)))
	sendMsg := banktypes.NewMsgSend(fromAddr, toAddr, amount)

	// Wrap message in Any
	msgAny, err := codectypes.NewAnyWithValue(sendMsg)
	if err != nil {
		return nil, nil, fmt.Errorf("cosmos: failed to create message any: %w", err)
	}

	// Decode public key
	pubKeyBytes, err := hex.DecodeString(pubKeyHex)
	if err != nil {
		return nil, nil, fmt.Errorf("cosmos: failed to decode pubkey: %w", err)
	}

	if len(pubKeyBytes) != 33 {
		return nil, nil, fmt.Errorf("cosmos: invalid pubkey length: expected 33 bytes, got %d", len(pubKeyBytes))
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("cosmos: failed to create pubkey any: %w", err)
	}

	// Create fee
	feeAmount, ok := math.NewIntFromString(DefaultFeeAmount)
	if !ok {
		return nil, nil, fmt.Errorf("cosmos: invalid fee amount")
	}
	fee := &tx.Fee{
		Amount:   cosmostypes.NewCoins(cosmostypes.NewCoin(AtomDenom, feeAmount)),
	...
	if err != nil {
		return nil, nil, fmt.Errorf("cosmos: failed to marshal tx body: %w", err)
	}

	authInfoBytes, err := s.cdc.Marshal(authInfo)
	if err != nil {
		return nil, nil, fmt.Errorf("cosmos: failed to marshal auth info: %w", err)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("cosmos: failed to marshal sign doc: %w", err)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("cosmos: failed to marshal tx: %w", err)
	}
...
func (s *SendService) GetSequence(ctx context.Context, address string) (uint64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("cosmos: failed to get account: %w", err)
	}
