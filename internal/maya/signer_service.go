...
// SignerService handles MayaChain transaction signing and broadcasting
...
// SignAndBroadcast signs a MayaChain transaction and broadcasts it
func (s *SignerService) SignAndBroadcast(
	...
	if err != nil {
		return "", fmt.Errorf("maya: failed to build keysign request: %w", err)
	}

	signatures, err := s.signer.Sign(ctx, keysignRequest)
	if err != nil {
		return "", fmt.Errorf("maya: failed to get signature: %w", err)
	}

	signedTxBytes, err := s.sdk.Sign(txData, signatures, pubKey)
	if err != nil {
		return "", fmt.Errorf("maya: failed to sign transaction: %w", err)
	}

	_, err = s.sdk.Broadcast(ctx, signedTxBytes)
	if err != nil {
		return "", fmt.Errorf("maya: failed to broadcast transaction: %w", err)
	}

	// MayaChain txHash is SHA256 of signed tx bytes (uppercase hex)
...
func (s *SignerService) buildKeysignRequest(
	...
	// For MayaChain (Cosmos-based), the hash to sign is SHA256 of the sign bytes
	...
	if err != nil {
		return types.PluginKeysignRequest{}, fmt.Errorf("maya: failed to create tx: %w", err)
	}
